...
public class SearchDeviceFromModule : MonoBehaviour
{
        ...
        if (!StaticVariable.navigate_from_JB_TSD_Detail)
        {
            ...
            if (module_Canvas.gameObject.activeSelf)
            {
                ...
                if (listDeviceFromModule.Count > 0)
                {
                    ...
                    StartCoroutine(InitValue()); // Gọi OnValueChange để cập nhật thông tin thiết bị đầu tiên
    ...
    {
        return StaticVariable.temp_ListDeviceInformationModel;
    }
    ...
    private void UpdateDeviceInformation(DeviceInformationModel device)
    {
        ...
        // if (StaticVariable.temp_ListJBInformationModel != null)
        // {
        //     Debug.LogError("temp_ListJBInformationModel is not null!");
        // }

        jBInformationModel_ConnectingToDevice = StaticVariable.temp_ListJBInformationModel.Find(jB => jB.Name == device.JBInformationModel.Name);
